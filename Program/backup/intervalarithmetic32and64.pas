...
function interval_mean(a: Interval) : Extended;
function compare_less(a, b: Interval) : Boolean;
function compare_less_or_equal(a, b: Interval) : Boolean;
function compare_equal(a, b: Interval) : Boolean;
...
implementation
  (*BS uses System.SysUtils, System.Math, Vcl.Dialogs;*)
...
BS*)
 ...
 (*BS
  ...
  begin
    ...
    (*BS if FormatSettings.DecimalSeparator=','*)
    if DecimalSeparator=','
  ...
  begin
    ...
    //BS if (Pos('.', sa1)>0) and (FormatSettings.DecimalSeparator=',')
   if (Pos('.', sa1)>0) and (DecimalSeparator=',')
      then sa1[Pos('.', sa1)]:=',';
    if(sa1[0]=' ') then delete(sa1,1,1);
    x:=StrToFloat(sa1);
    if Pos('e', sa1)>0
  ...
  begin
    ...
    begin
      if interval_mean(a) < interval_mean(b) then
        Result := True
      else Result := False
    end;
